# -*- coding: utf-8 -*-
"""
Script for evaluating GBC.

@TODO: All the three evaluation scripts should be merged... if have time..
"""
...
def main(_):
    ...
    top1_correct = 0
    num_total_rel = 0
    classifier = cPickle.load(open(FLAGS.checkpoint_file, "r"))
    for ind, (img_fname, ann) in enumerate(annotations.iteritems()):
        ...
        if FLAGS.cal_vscore:
            ...
            for rel in ann:
                predictions = post_process_vscore(np.squeeze(classifier.predict_proba([[rel["object"]["category"], rel["subject"]["category"]] + rel["object"]["bbox"] + rel["subject"]["bbox"]])))
